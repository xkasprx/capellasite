...
async function loadDashboard(){
	let username = cookies && cookies.firstName || `Member`;
	...
	greeting.innerHTML = `<h2>Greetings, ${username}.</h2>`;
	...
	new gridjs.Grid({
		...
		data: await(async(a = []) => {
			...
			for(let i = 0; i < info.length; i++){
				...
				a.push([dataRow.company, dataRow.title, `${dataRow.from}-${dataRow.current ? `Present` : dataRow.to}`]);
			}
			return a;
		})(),
	}).render(expTable);
	...
	new gridjs.Grid({
		...
		data: await(async(a = []) => {
			...
			for(let i = 0; i < info.length; i++){
				...
				a.push([dataRow.school, dataRow.degree, `${dataRow.from}-${dataRow.current ? `Present` : dataRow.to}`]);
			}
			return a;
		})(),
	}).render(eduTable);
